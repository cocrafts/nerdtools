...
- **A card can have more than one writer.** It is untracked, so there is no merge and no history: edit your own sections in place, re-read it immediately before writing rather than at the top of the turn, and never write the file whole — a whole-file write from a stale read deletes the other writer's section silently.
