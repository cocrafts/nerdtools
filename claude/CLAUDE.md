...
- **Ask with the options already enumerated, not openly.** `AskUserQuestion` renders them. "Land / land then re-gate / leave it" is answered in one beat; "should I land?" comes back as "push đi" and leaves the session guessing what it covered. The user's minutes are the scarcest budget in any setup that runs more than one session, and a closed question spends one instead of three.
