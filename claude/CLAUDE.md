...
- **A question earns its place before it is sent.** At most two questions per call, the blocking one first. The prose above it names every reference model read at source (file and proc — a doc's summary is not a reading), with pros, cons and measured numbers. A decision outside the task the user gave is reported, not asked; one a rule already answers is acted on, naming the rule. Every command handed to the user was run first. `hooks/ask-guard.sh` enforces the mechanical part; `playbooks/asking.md` holds the data behind it.
