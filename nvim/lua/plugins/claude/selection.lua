...
function M.has_selection_changed(new_selection)
    ...
    if old.selection.start.line ~= new_selection.selection.start.line or
       old.selection.start.character ~= new_selection.selection.start.character or
       old.selection["end"].line ~= new_selection.selection["end"].line or
       old.selection["end"].character ~= new_selection.selection["end"].character then
        return true
    end
...
function M.update_selection()
    ...
    if mode == "v" or mode == "V" or mode == "\22" then
        ...
        if last_visual and last_visual.bufnr == current_buf and
           not state.demotion_timer then
...
function M.handle_selection_demotion(original_bufnr)
    ...
    if mode == "v" or mode == "V" or mode == "\22" then
        if state.last_active_visual_selection and
           state.last_active_visual_selection.bufnr == original_bufnr then
            state.last_active_visual_selection = nil
        end
        return
    end
