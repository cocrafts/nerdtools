		...
		vim.schedule(function()
			...
			if message.method == "tools/call" and message.params then
				...
				if message.params.name == "openFile" then
					vim.notify(
						"[Claude IDE] OpenFile params: " .. vim.inspect(message.params.arguments),
						vim.log.levels.INFO
					)
				end
			end
...
function M.handle_poll_commands(id, params)
	...
	if #commands > 0 then
		vim.schedule(function()
			vim.notify(string.format("[Claude IDE] Sending %d queued commands", #commands), vim.log.levels.DEBUG)
		end)
	end
...
function M.send_diff_response(tab_name, decision, content)
	...
	vim.notify(
		string.format("[Claude IDE] Sending deferred response: %s - %s", decision, tab_name),
		vim.log.levels.WARN
	)
	...
	if pending.client_id then
		...
		vim.notify(
			string.format("[Claude IDE] Sent deferred response to client: %s", pending.client_id),
			vim.log.levels.INFO
		)
	else
